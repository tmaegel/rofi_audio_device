...
@mockenv(ROFI_INFO="card_id=0,card_name=name,type=card")
...
@mockenv(ROFI_INFO="card_id=0,card_name=name,type=card,invalid=abc")
...
@mockenv(ROFI_INFO="card_id=0,card_name=name")
def test_rofi_get_info__invalid_3():
    with pytest.raises(SystemExit) as wrapped_exit:
        _ = RofiWrapper.get_info()
    assert wrapped_exit.type == SystemExit
    assert wrapped_exit.value.code == 1
